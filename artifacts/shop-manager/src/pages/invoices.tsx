...
export default function Invoices() {
  ...
  const filtered = (invoices ?? []).filter((inv) => tab === "all" || inv.type === tab);
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = async () => {
      ...
      toast({ title: "Invoice scanned", description: result.vendorOrCustomer ? `Detected: ${result.vendorOrCustomer}` : "Data extracted" });
    };
    ...
    <div className="p-6 space-y-5">
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
        <TabsList>
          ...
          <TabsTrigger value="purchase" data-testid="tab-purchase">Purchases</TabsTrigger>
          <TabsTrigger value="sale" data-testid="tab-sale">Sales</TabsTrigger>
        </TabsList>
      </Tabs>
            ...
            <tbody className="divide-y divide-card-border">
              {filtered.slice().reverse().map((inv) => (
                <tr key={inv.id} className="hover:bg-accent/30 transition-colors" data-testid={`row-invoice-${inv.id}`}>
                  ...
                  <td className="px-4 py-3 text-right font-medium text-foreground">{inv.amount != null ? `AED ${inv.amount.toFixed(2)}` : "—"}</td>
          ...
          >
            {parseImage.isPending ? (
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-sm text-muted-foreground">Scanning invoice...</p>
              </div>
            ) : imagePreview ? (
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-sm text-muted-foreground">Upload invoice photo — AI will extract details</p>
              </div>
            )}
            <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleImageUpload} data-testid="input-invoice-image" />
          </div>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField control={form.control} name="type" render={({ field }) => (
                <FormItem>
                  ...
                  <Select value={field.value} onValueChange={field.onChange}>
                    <FormControl><SelectTrigger data-testid="select-invoice-type"><SelectValue /></SelectTrigger></FormControl>
                    <SelectContent>
              ...
              <FormField control={form.control} name="vendorOrCustomer" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Company or person name" {...field} data-testid="input-invoice-vendor" /></FormControl>
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="notes" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Any notes..." {...field} data-testid="input-invoice-notes" /></FormControl>
                </FormItem>
              )} />
              <div className="flex justify-end gap-2 pt-2">
                <Button type="button" variant="ghost" onClick={() => setDialogOpen(false)}>Cancel</Button>
                <Button type="submit" disabled={createInvoice.isPending} data-testid="button-save-invoice">Save Invoice</Button>
              </div>
            </form>
          </Form>
        </DialogContent>
