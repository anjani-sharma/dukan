...
export default function Dashboard() {
  ...
  const fmt = (n?: number) => `AED ${(n ?? 0).toLocaleString("en-AE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
            ...
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={chart} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v: number) => [`AED ${v.toFixed(2)}`, "Sales"]}
                />
            ...
            <div className="space-y-3">
              {topCustomers.slice(0, 6).map((c) => (
                <div key={c.id} className="flex items-center justify-between" data-testid={`debtor-${c.id}`}>
                  ...
                  <div className="text-sm font-bold text-amber-400 ml-4 flex-shrink-0">AED {c.outstandingBalance.toFixed(0)}</div>
                </div>
              ))}
            </div>
          ) : (
          ...
          <div className="space-y-1">
            {activity.map((item) => (
              <div key={item.id} className="flex items-center gap-4 py-2.5 px-3 rounded-lg hover:bg-accent/50 transition-colors" data-testid={`activity-${item.id}`}>
                ...
                {item.amount != null && (
                  <div className="text-sm font-semibold text-foreground flex-shrink-0">AED {item.amount.toFixed(2)}</div>
                )}
              </div>
            ))}
          </div>
