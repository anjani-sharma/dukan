          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-muted rounded-xl p-4 text-center">
                ...
                <div className="text-lg font-bold text-foreground">AED {customer.totalCredit.toFixed(0)}</div>
              </div>
              <div className="bg-muted rounded-xl p-4 text-center">
                <div className="text-xs text-muted-foreground mb-1">Paid</div>
                <div className="text-lg font-bold text-emerald-400">AED {customer.totalPaid.toFixed(0)}</div>
              </div>
              <div className="bg-amber-400/10 border border-amber-400/20 rounded-xl p-4 text-center">
                ...
                <div className="text-lg font-bold text-amber-400">AED {customer.outstandingBalance.toFixed(0)}</div>
              </div>
            </div>
                ...
                <div className="space-y-2">
                  {payments.map((p) => (
                    <div key={p.id} className="flex items-center justify-between bg-muted/50 rounded-lg px-4 py-3" data-testid={`payment-${p.id}`}>
                      <div>
                        <div className="text-sm font-medium text-emerald-400">AED {p.amount.toFixed(2)}</div>
                  ...
                  {customer.sales.map((s) => (
                    <div key={s.id} className="bg-muted/50 rounded-lg px-4 py-3" data-testid={`sale-history-${s.id}`}>
                      <div className="flex items-center justify-between mb-1">
                        <div className="text-sm font-medium text-foreground">AED {s.totalAmount.toFixed(2)}</div>
                        <div className="text-xs text-muted-foreground">{format(new Date(s.createdAt), "MMM d, yyyy")}</div>
                      </div>
                      {s.creditAmount > 0 && (
                        <div className="text-xs text-amber-400">Outstanding: AED {s.creditAmount.toFixed(2)}</div>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-4">
                {c.outstandingBalance > 0 ? (
                  <div className="text-right">
                    <div className="text-sm font-bold text-amber-400">AED {c.outstandingBalance.toFixed(2)}</div>
