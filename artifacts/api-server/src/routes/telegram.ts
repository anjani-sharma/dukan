...
async function getDashboardSummaryText(): Promise<string> {
  ...
  msg += `💳 Outstanding Balances: ₹${outstanding.toFixed(2)}\n`;
...
router.post("/telegram/webhook", async (req, res) => {
  ...
  try {
    ...
    if (text === "/outstanding") {
      ...
      debtors.forEach((d, i) => {
        msg += `${i + 1}. ${d.name}${d.phone ? ` (${d.phone})` : ""}: ₹${d.balance.toFixed(2)}\n`;
      });
...
Return only valid JSON.`,
      ...
      items.forEach((i) => {
        msg += `  • ${i.productName} × ${i.quantity} @ ₹${i.unitPrice} = ₹${(i.quantity * i.unitPrice).toFixed(2)}\n`;
      });
    ...
    if (photo) {
      ...
      if (data.items?.length) {
        ...
        (data.items as { name: string; quantity: number; unitPrice: number }[]).slice(0, 5).forEach((i) => {
          msg += `  • ${i.name} × ${i.quantity} @ ₹${i.unitPrice}\n`;
        });
      }
      await sendTelegramMessage(chatId, msg);
      return;
